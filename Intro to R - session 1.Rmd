...
Welcome to the R bootcamp for "ARMS to reefs: A new tool to restore coral reef biodiversity, fisheries yields, and human health in Madagascar".
During this bootcamp we will introduce you to R and RStudio, show you how to imput and wrangle data, and introduce you to the analysis and visualization of biodiversity, catch and quantitative social data. 

This course assumes that you have never done any computer programming before, so we will start from the beginning. 

The first two days are introductory courses so you get to know how to work with R and RStudio. At times, you might find this to be frustrating and tedious. We acknowledge inmersing yourselves into computer programming can be a steep learning curve (we have been there...), but believe us when we say it is totally worth it! We are here to help you navigate this process. 

The third day we will introduce you to biodiversity metrics......... (Thomas to fill in?)

The fourth day, using real catch statistics from Madagascar,  we will work on...... (Daniel to fill in)

Finally, the last day of the course, we will use real household surveys from X to .....(me to fill in)
...
Let's start by downloading R and RStudio on your computers. 
...
2+2
```
an press enter, R will return the value of 4. R "heard" you, understood what you wanted when you said “2+2”, carried out the addition, and told you the answer.

Let's move on to RStudio. To do so, first we will close R. We do this by typing:

```{r}
q()
```
When you quit R, it will ask you if you want to save the workspace (that is, all of the variables you have defned in this session); for now (and in general), say "no" in order to avoid clutter.

Similar to how we opened R, now we open RStudio (click the RStudio icon on your desktop or search for RStudio in your aplications). 
On the left side, you may find something familar. That is right, it is the console window, just like the one we saw in R.

However, in contrast to R, in RStudio you have other panels. 
On the bottom right, you will likely see some files. This is your working directory (where files related to the project will be or will be stored).
...
class(x)
```
This tell you x is a numeric variable. You could instead create a character string. For example:
```{r}
x1="a"
class(x1)
```
...
R can tell if you haven't finished a command and will give you a continuation prompt (+) to let you know you havent finished. For example, type z=3+ and press Enter:
```{r}
z=3+
```
The prompt "+" appears, detailing that R is missing some information to finish assigning your variable. For example:
```{r}
5
```
...
You can also remove variables by suing the function rm(). For example, if we wanted to delete the variable "x" from our environment, we would type:
```{r}
rm(x)
```
You will see that "x" no longer appears in your environment, and if you type "x" an error message will appear: "Error: object 'x' not found".
```{r}
x
```
...
You can now type everything you have done so far in the Console window in the script file- that way you ensure you can reproduce everything in the future. In the scrpt file type:
```{r}
getwd()
x=2+2
x
...
Remember that these scripts need to be in the working directory, or R won’t see them.
...
You do this with the "#" character. Try it. Go to the command linein your Console, and type a comment without the #. For example, write "This is a comment" and press Enter:
```{r}
This is a comment
```
...
Now go ahead and comment your script accordingly. 
